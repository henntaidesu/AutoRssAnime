...
def qbittorrent(save_path, torrent_file, quarter):
    ...
    if response.status_code == 200:
        ...
        if response.status_code == 200:
            ...
            print(f"添加下载任务失败: {response.text}")
    else:
        ...
        for i in data:
            ...
            if quarter:
                ...
                if torrent_from == 'nyaa':
                    ...
                    if qbittorrent(save_path, torrent_file, quarter):
...
def main():
    while True:
        auto_download()
        Log().write_log(f'休眠{stop_time}秒', 'info')
